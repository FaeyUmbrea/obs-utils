...
/**
 * The marquee animated roll banner — used as both the migration's seed overlay
 * and the only built-in starter the editor's template gallery surfaces.
 *
 * 1920×1080 canvas. The card sits in the lower-third so it doesn't fight the
 * chat box. On `core.onPlayerRoll`, the static idle track jumps to the reveal
 * track which slides + fades the card in, holds, then exits back to idle.
 */
function makeRollRevealBanner(): OverlayData {
	const card = comp('pt', '', 0, 0, 480, 140,
		'background:linear-gradient(140deg,#1c1330 0%,#2a1f4f 50%,#3a1d2e 100%);'
	...
	const dieIcon = comp('fai', 'fas fa-dice-d20', 18, 18, 64, 64,
		'color:#ffd166;font-size:64px;filter:drop-shadow(0 2px 6px rgba(255,209,102,0.45));');
	const actorName = comp('pt', 'trigger.actor.name', 100, 20, 364, 28,
		'font-weight:700;font-size:20px;color:#f6f1e7;text-shadow:0 1px 3px rgba(0,0,0,0.7);letter-spacing:0.3px;');
	const formula = comp('pt', 'trigger.formula', 100, 50, 240, 22,
		'font-size:14px;color:#bcb1a4;font-family:monospace;');
	const total = comp('pt', 'trigger.total', 340, 52, 124, 72,
		'font-weight:800;font-size:56px;color:#ffd166;text-align:right;line-height:1;'
		+ 'text-shadow:0 2px 8px rgba(255,209,102,0.45);font-feature-settings:"tnum";');

	const o = new OverlayData('wysiwyg', [card, dieIcon, actorName, formula, total], '',
		{ w: 1920, h: 1080 }, 'Roll Reveal');
	const cx = (1920 - 480) / 2;
	const cy = 1080 - 220;
	...
	const anim: OverlayAnimationData = {
		tracks: [
			...
			track(revealId, 'Reveal', 2800,
				{ type: 'transition-on-end', toTrackId: idleId, toTime: 0 },
				all.map(revealLane)),
		],
...
/** Single sample used by the migration's `streamOverlays` initializer. */
...
// the generic default. The editor's template gallery surfaces whichever set is
