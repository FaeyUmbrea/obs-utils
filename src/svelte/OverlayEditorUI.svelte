...
<div class="grid">
<div class="preview">
    <InformationOverlay overlays={overlays} actorIDs={actorIDs}/>
</div>
...
<div class="editor">
    ...
    <nav class="tabs" data-group="primary-tabs">
        {#each overlays as overlay, index (overlays.indexOf(overlay)) }
            ...
            <a class="item {(index == activeIndex) ? "active": ""}" data-tab={index} on:click={() => changeTab(index)}><i class="fas fa-dice-d20"></i>{index}</a>
        {/each}
    </nav>
