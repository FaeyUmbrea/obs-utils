...
<script lang='ts'>
	...
	// ─── Scene presets state ───────────────────────────────────────────────
	// Tracks the currently-viewed scene so the Presets tab re-renders when
	// the user switches scenes without having to reopen the Director.
	...
	function onUpdateScene(scene: any) {
		// Re-read if the scene that changed is the one we're viewing.
	...
	async function applyPreset(p: CameraPreset) {
