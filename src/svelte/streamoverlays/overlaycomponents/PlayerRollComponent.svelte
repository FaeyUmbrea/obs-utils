<script>
    ...
    let rollDelay = $pre ? $preRollDelay + $preRollFadeIn + $preRollStay + $preRollFadeOut : 0;
    ...
    function recalculateRollDelay() {
        rollDelay = $pre ? $preRollDelay + $preRollFadeIn + $preRollStay + $preRollFadeOut : 0;
    }
    ...
    {#if postRollShow}
        {#if $post && $postRollImage}
            <img class="after layer" in:fade="{{delay:postRollDelay,duration:$postRollFadeIn}}"
                 out:fade="{{delay:$postRollStay,duration:$postRollFadeOut}}" src={$postRollImage}
                 on:introend={()=>postRollShow=false} alt="post roll"/>
        {/if}
    {/if}
</div>